...
ATRIBUICAO:  ID ATRIB LITERALINT SCOL {printf("atribuicao\n");}
;

IFZEIRA: 
	IF OPAR ID EQUAL LITERALINT CPAR OBRAC AQUIDENTRO CBRAC SCOL{printf("IF(ID = LITERALINT);\n");}
	| IF OPAR ID EQUAL ID CPAR OBRAC CBRAC SCOL{printf("IF(ID == ID);\n");}
;
